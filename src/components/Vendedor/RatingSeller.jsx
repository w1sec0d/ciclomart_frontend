...
const RatingSeller = ({
  ...
  const encabezado = (nota) => {
    const listaEncabezados = [
      'Incumplido y poco confiable',
      'Poco cumplido y confiable',
      'Cumplido',
      'cumplido y confiable',
      'Muy cumplido y confiable',
    ]

    return listaEncabezados[nota - 1]
  }

  return (
    <div className="flex  flex-col space-y-4 w-full mt-3">
      <div className="flex flex-row space-x-6 items-center ">
        <h2 className="text-1xl font-bold ">{encabezado(rating)}</h2>
        <div className="flex space-x-1">
          <FaStar size={26} color="#ffc107" />
          <p className="text-1xl ">
            {rating} <strong className="text-green-700">Estrellas</strong>
          </p>
        </div>
        <div className="flex space-x-1">
      ...
      <div className="flex flex-row mt-2">
        ...
        <div className="flex-grow mx-10 space-y-2">
          ...
          <Link className="text-sm text-blue-500" to={`/product/${idProducto}`}>
            Ver producto
          </Link>
